...
class ActionModel(crocoddyl.ActionModelAbstract):
    ...
    def __init__(self, nx=12, nu=12, mu=0.8, log=True):
        ...
        self.footholds = np.array([[0.19, 0.19, -0.19, -0.19], [0.15005, -0.15005, 0.15005, -0.15005],
                                   [0.0, 0.0, 0.0, 0.0]])
    ...
    def update_derivative_B(self, u):
        ...
        for i in range(4):
            if (self.S[i] != 0):
                self.derivative_B[-3:, 0] = -np.dot(
                    self.I_inv, self.dt * np.cross([1, 0, 0], [u[3 * i], u[3 * i + 1], u[3 * i + 2]]))  # \x
                self.derivative_B[-3:, 1] = -np.dot(
                    self.I_inv, self.dt * np.cross([0, 1, 0], [u[3 * i], u[3 * i + 1], u[3 * i + 2]]))  # \y
                self.derivative_B[-3:, 2] = -np.dot(
                    self.I_inv, self.dt * np.cross([0, 0, 1], [u[3 * i], u[3 * i + 1], u[3 * i + 2]]))  # \z
